...
const I18N = {
  uk: {/* =============================================
   КОЗАЦЬКИЙ СЕРІАЛ — main.js + КОНСОЛЬ-ЛОГЕРА
   ============================================= */
...
function setLang(lang) {
  ...
  if (taskSavedTime && now < parseInt(taskSavedTime)) {
    ...
    console.log(`[HOLD] Знайдено активний холд до часової мітки: ${taskSavedTime} (${savedTimeString}). Скло БЛОКУЄТЬСЯ.`);
    ...
    if (globalGlass) {
      ...
      console.log("[GLASS] Клас 'impenetrable' успішно додано до глобального скла.");
    }
  } else {
    console.log("[HOLD] Активного холда немає або термін дії вийшов. Сторінка відкрита для кліків.");
    if (globalGlass) {
      globalGlass.classList.remove('impenetrable');
      console.log("[GLASS] Клас 'impenetrable' видалено (якщо він був). Скло прозоре.");
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log("[SYSTEM] DOM повністю побудовано. Підключення прослуховувачів подій.");
  let isUserClickedAd = false;
  let blurTimer = null;

  function lockSystemForDay() {
    console.log("[HOLD] Запуск функції блокування системи на 24 години...");
    ...
    console.log(`[HOLD] Систему заблоковано в LocalStorage до ${timeString}. Оновлюємо інтерфейс...`);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (globalGlass && globalGlass.classList.contains('impenetrable')) {
      console.log("[CLICK CONTROL] Спроба кліку під час активного холду відхилена скриптом.");
      e.stopPropagation();
      e.preventDefault();
      return;
    }
    ...
    if (isClickInAdZone) {
      console.log(`[CLICK CONTROL] Клік зафіксовано в рекламній зоні! Ціль:`, e.target);
  ...
  window.addEventListener('blur', () => {
    console.log(`[FOCUS CONTROL] Сторінка втратила фокус (blur). Намір кліку по рекламі: ${isUserClickedAd}`);
    if (isUserClickedAd) {
      console.log("[FOCUS CONTROL] Запуск таймера очікування на сторінці реклами (2.5 сек)...");
      blurTimer = setTimeout(() => {
        window._isRewardApproved = true; 
        console.log("[FOCUS CONTROL] Таймер витримано! Винагороду схвалено (window._isRewardApproved = true).");
      }, 2500); 
    }
  });
  ...
  window.addEventListener('focus', () => {
    console.log("[FOCUS CONTROL] Сторінка знову у фокусі (focus).");
    if (isUserClickedAd) {
      console.log(`[FOCUS CONTROL] Очищення таймера. Результат схвалення винагороди: ${window._isRewardApproved}`);
      clearTimeout(blurTimer);
      
      if (window._isRewardApproved) {
        lockSystemForDay(); 
      } else {
        console.log("[FOCUS CONTROL] Користувач повернувся занадто швидко. Холд не активовано.");
      }
...
(function () {
  ...
  console.log(`[BOOT] Початкове визначення мови. Збережено: ${saved}, URL: ${urlLang}. Обрано: ${detected}`);
